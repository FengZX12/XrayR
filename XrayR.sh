...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/FengZX12/XrayR/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/FengZX12/XrayR/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/FengZX12/XrayR ---
  ${green}0.${plain} 修改配置
————————————————
